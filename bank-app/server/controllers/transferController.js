...
// Transfer funds to another user's account identified by routing + account number.
export async function crossUserTransfer(req, res, next) {
  try {
    const { fromAccountId, routingNumber, accountNumber, amount } = req.body;
    const userId = req.user._id;

    if (!fromAccountId || !routingNumber || !accountNumber || amount == null) {
      return res.status(400).json({
        success: false,
        message: 'fromAccountId, routingNumber, accountNumber, and amount are required.',
      });
    }
    ...
    // Validate sender owns the source account
    const fromCheck = await BankAccount.findOne({ _id: fromAccountId, user: userId });
    if (!fromCheck) return res.status(404).json({ success: false, message: 'Source account not found or not owned by you.' });

    // Look up destination by routing + account number (accountNumber has select:false but is queryable)
    const toCheck = await BankAccount.findOne({ routingNumber: routingNumber.trim(), accountNumber: accountNumber.trim() })
      .populate('user', '_id firstName lastName email');
    if (!toCheck) {
      return res.status(404).json({ success: false, message: 'No account found with that routing and account number.' });
    }

    if (String(fromCheck._id) === String(toCheck._id)) {
    ...
    const recipientDisplay = `${toCheck.user?.firstName ?? ''} ${toCheck.user?.lastName ?? ''}`.trim() || 'recipient';
    ...
    if (fraud.outcome === 'blocked') {
      await Transaction.create({
        ...
        category:        'transfer',
        amount:          parsedAmount,
        description:     `BLOCKED: External transfer to ••••${toCheck.last4}`,
        status:          'failed',
        referenceNumber: ref,
        ...
        pendingMeta:     { fromAccountId, toAccountId: String(toCheck._id), crossUser: true },
      });

      if (io) {
        io.to('admins').emit('admin:securityAlert', {
          type: 'BLOCKED_TRANSFER', severity: 'critical',
          riskScore: fraud.riskScore, riskLevel: fraud.riskLevel,
          flags: fraud.fraudFlags, userId: String(userId),
          userName: `${req.user.firstName} ${req.user.lastName}`,
          amount: parsedAmount, ref, timestamp: now,
        });
      }
      ...
      return res.status(403).json({
        success: false, fraudBlocked: true,
    ...
    if (fraud.outcome === 'pending-review') {
      const pendingTx = await Transaction.create({
        ...
        category:        'transfer',
        amount:          parsedAmount,
        description:     `PENDING REVIEW: External transfer to ••••${toCheck.last4}`,
        status:          'pending',
        referenceNumber: ref,
        balanceAfter:    fromCheck.availableBalance,
        transactionDate: now,
      ...
      if (io) {
        io.to('admins').emit('admin:securityAlert', {
          type: 'PENDING_REVIEW', severity: 'high',
          riskScore: fraud.riskScore, riskLevel: fraud.riskLevel,
          flags: fraud.fraudFlags, userId: String(userId),
          userName: `${req.user.firstName} ${req.user.lastName}`,
          amount: parsedAmount, ref, transactionId: String(pendingTx._id), timestamp: now,
        });
      }
      ...
      createNotification({
        ...
        message:  `Your transfer of ${amt} is being reviewed for security purposes.`,
        type:     'warning',
        category: 'security',
      }).catch(() => {});

      return res.status(202).json({
        success: true, pendingReview: true,
        riskLevel: fraud.riskLevel, referenceNumber: ref,
        message: 'Transfer is pending security review. You will be notified once it is processed.',
      });
    }

    // ── EXECUTE via banking engine (double-entry) ─────────────────────
    let engineResult;
    try {
      engineResult = await engine.transferFunds({
        fromAccountId,
        toAccountId:   String(toCheck._id),
        amount:        parsedAmount,
        description:   `External transfer to ${recipientDisplay} ••••${toCheck.last4}`,
        ...
        riskScore:     fraud.riskScore,
        metadata:      { originalRef: ref, crossUser: true },
      });
    } catch (engineErr) {
      ...
      console.error('[transferController] cross-user engine error:', engineErr.message);
      return res.status(500).json({ success: false, message: 'Transfer could not be completed. Please try again.' });
    }
    ...
    // Post-response: notifications for both parties
    if (io) {
      io.to('admins').emit('admin:newTransfer', {
        referenceNumber: engineResult.debitTransaction.referenceNumber,
        amount: parsedAmount, riskScore: fraud.riskScore, riskLevel: fraud.riskLevel,
    ...
    Promise.all([
      createNotification({
        userId,
        title:    'Transfer sent',
        message:  `You sent ${amt} to ${recipientDisplay}.`,
        type:     'success',
        category: 'transfer',
        metadata: { referenceNumber: ref, amount: parsedAmount },
      }),
      createNotification({
        userId:   toCheck.user._id,
        title:    'Transfer received',
        message:  `You received ${amt} in your account ••••${toCheck.last4}.`,
        type:     'success',
        category: 'transfer',
